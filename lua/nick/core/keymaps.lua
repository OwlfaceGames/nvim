...
-- swap p and shift p binds for visual mode for 
-- normal paste over functionality
...
end, { desc = 'Telescope word under cursor' })

-- live grep word under cursor
-- prefills search but doesnt return so you can edit the search term
vim.keymap.set('n', '<leader>fc', function()
  require('telescope.builtin').live_grep({
    default_text = vim.fn.expand('<cword>'),
  })
end, { desc = 'Telescope prefill live grep word under cursor' })

-- telescope fuzzy find references
keymap.set('n', '<leader>fr', function()
    require('telescope.builtin').lsp_references({
        position_encoding = vim.lsp.get_clients({
            bufnr = 0
        })[1].offset_encoding
    })
...
-- no neck pain
keymap.set("n", "<leader>nn", "<cmd>NoNeckPain<CR>", {
...
keymap.set('n', '<leader>lrn', ':setlocal number relativenumber <CR>', {
...
keymap.set('n', '<leader>qn', ':cnext<CR>', {
    desc = "next item in quick list"
})
keymap.set('n', '<leader>qp', ':cprev<CR>', {
    desc = "previous item in quick list"
})
...
-- set color column
keymap.set('n', '<leader>lc', ':set colorcolumn=80<CR>', {
    silent = true,
    desc = "turn on color column"
})
keymap.set('n', '<leader>nlc', ':set colorcolumn=<CR>', {
    silent = true ,
    desc = "turn color column off"
})

-- set winbar
keymap.set('n', '<leader>wb', ':set winbar=%f<CR>', {
    silent = true,
    desc = "turn winbar on (filename)"
})
keymap.set('n', '<leader>nwb', ':set winbar=<CR>', {
    silent = true,
    desc = "turn winbar off (filename)"
})
